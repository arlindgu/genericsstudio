  ...
  return (
    <section className="py-24 border-t border-border" id="leistungen">
      <div className="container px-10 max-w-7xl mx-auto flex flex-col gap-12">
        ...
        <AnimateIn delay={0.1}>
          <div className="-mx-10 grid grid-cols-1 lg:grid-cols-2 border-border">
            {items.map((service, i) => (
              <Link
                key={service.number}
                href={service.href}
                className={cn(
                  "group p-10 flex flex-col gap-6 hover:bg-foreground hover:text-background transition-colors duration-500 border-b lg:border-0 last:border-0 lg:border-r lg:last:border-r-0",
                  i % 2 !== 1 && "lg:border-r",
                  i < items.length - 2 && "lg:border-b",
                )}
              >
